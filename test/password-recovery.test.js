...
test('password reset URLs keep the Better Auth token in the browser-only fragment', () => {
  ...
  assert.equal(fragmentParams.get('token'), 'better-auth-token');
  assert.equal(fragmentParams.size, 1);
...
test('password reset composition depends only on the provider-neutral email contract', async () => {
  const passwordResetSource = await readFile(
    new URL('../src/lib/server/email/password-reset.ts', import.meta.url),
    'utf8'
  );

  assert.match(passwordResetSource, /from '\.\/transactional\.ts'/);
  assert.doesNotMatch(passwordResetSource, /from '\.\/resend\.ts'/);
});
...
test('auth configuration preserves closed enrollment and enables secure Better Auth reset semantics', async () => {
  ...
  assert.match(authSource, /sendEmail:\s*sendTransactionalEmail/);
...
test('forgot-password and reset-password pages preserve anti-enumeration and token-rendering contracts', async () => {
  ...
  assert.match(resetPageSource, /window\.location\.hash/);
  assert.doesNotMatch(resetPageSource, /window\.location\.search/);
