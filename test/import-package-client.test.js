...
test('media failure is warning-only and successful Blob URLs are revoked on invalidation', async () => {
  const urls = [];
  const revoked = [];
  const previewModel = { cases: [{ assets: [{ asset: { id: 'asset-ready', operation: 'create', mediaPath: 'media/ready.png', mimeType: 'image/png' } }, { asset: { id: 'asset-bad', operation: 'create', mediaPath: 'media/bad.png', mimeType: 'image/png' } }] }] };
  const controller = createImportPreviewController({
