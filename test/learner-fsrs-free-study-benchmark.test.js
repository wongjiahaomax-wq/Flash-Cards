...
test('Part E benchmark measures the four-row Free completion bundle and bounded receipt cleanup', () => {
  const result = runFreeStudyBenchmark({ completionCount: 40, cleanupLimit: 10 });
  ...
  assert.equal(result.rows.receiptRows, 40);
  assert.equal(result.rows.encounterFreeTimesStudied, 40);
  assert.equal(result.rows.learnerFreeCompleted, 40);
  assert.equal(result.cleanup.deleted, 40);
