<script>
  ...
  onMount(() => {
    ...
    // Fragments are not sent in the initial request. Remove the token from the
    // visible URL immediately after capturing it into browser memory.
  ...
  function resetErrorMessage(error) {
    if (error && typeof error === 'object' && 'code' in error) {
      if (error.code === 'INVALID_TOKEN') return 'This password reset link is invalid or has expired.';
      if (error.code === 'PASSWORD_TOO_SHORT') return 'That password is too short.';
