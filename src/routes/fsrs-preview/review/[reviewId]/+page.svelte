<script>
  ...
  async function completeReview(rating = null) {
    ...
    completionError = '';
    try {
      const response = await fetch(`/fsrs-preview/api/complete/${data.review.id}`, {
        method: 'POST',
...
<svelte:head>
  <title>Active FSRS Review | Flash-Cards</title>
</svelte:head>

<main class="shell review-shell">
  <nav class="review-nav" aria-label="Preview navigation">
    <a href="/fsrs-preview">← FSRS preview</a>
