<script>
  ...
  function confirmBulkTopicSystemMove(event) {
    const count = selectedCaseIds.length;
    if (!count || !window.confirm(`Move the Primary Topics for ${count} selected Case${count === 1 ? '' : 's'} globally? This changes the global Topic hierarchy for every Case using those Topics.`)) event.preventDefault();
  }
  ...
  <form method="POST" action={actionHref(inactiveView ? 'bulkRestoreCases' : 'bulkPromoteTopic')}>
    ...
    {:else}
      <div class="bulk-toolbar">
        ...
        <label class="bulk-system">System<select name="system_id" required disabled={!selectedCaseIds.length}><option value="">Choose a System</option>{#each data.topicParents.filter((option) => option.kind === 'system') as system}<option value={system.id}>{system.name}</option>{/each}</select></label>
        <button class="button warning" type="submit" formaction={actionHref('bulkMoveCaseTopicsToSystem')} formnovalidate disabled={!selectedCaseIds.length} onclick={confirmBulkTopicSystemMove}>Move Topics globally</button>
