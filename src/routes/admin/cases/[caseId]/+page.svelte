<script>
  ...
  function showImage(asset, subtitle = '') {
    ...
    viewerImage = {
      src: asset.imageUrl,
      alt: asset.altText ?? '',
      title: asset.originalFilename ?? asset.assetId ?? 'Teaching image',
      subtitle
    };
  }
...
{:else}
  ...
  <nav class="section-nav" aria-label="Case editor sections">
    <a href="#topics">Topics <span>{selectedCase.topics.length}</span></a>
    <a href="#case">Case</a>
    <a href="#images">Images <span>{selectedCase.attached.length + selectedCase.stimulusGroups.reduce((count, group) => count + group.options.length, 0)}</span></a>
    <a href="#questions">Case questions <span>{selectedCase.questions.length}</span></a>
    <a href="#preview">Preview</a>
  </nav>

  <section id="topics" class="panel stack">
    <div>
      <p class="eyebrow">Learner routing</p>
      <h2>Topics <span class="count">{selectedCase.topics.length}</span></h2>
      <p class="muted">The Primary/default Topic is the Case's canonical classification. Additional Study Topics are separate learner routes and may be selected during study.</p>
    </div>
    ...
    <div class="topic-primary">
      ...
      <div class="topic-primary-current">
        {#if primaryTopic}
          <a href={'/admin/topics/' + primaryTopic.id}>{primaryTopic.name}</a>
          {#if !primaryTopic.isActive}<span class="status-badge inactive">Topic inactive — select an active replacement</span>{/if}
        {:else}<span class="form-error inline-error">No primary Topic is attached. Select an active replacement below.</span>{/if}
      </div>
      <form method="POST" action="?/updateCase" class="topic-primary-form">
        <input type="hidden" name="case_id" value={selectedCase.case.id} />
        <input type="hidden" name="title" value={selectedCase.case.title} />
        <input type="hidden" name="vignette_md" value={selectedCase.case.vignetteMd ?? ''} />
        <input type="hidden" name="question_selection_mode" value={selectedCase.case.questionSelectionMode} />
        <input type="hidden" name="question_count" value={selectedCase.case.questionCount ?? ''} />
        <label class="topic-select-label">Change primary/default Topic<select name="concept_id" required>
          {#each data.concepts as concept}<option value={concept.id} selected={concept.id === selectedCase.case.conceptId}>{concept.name}</option>{/each}
          {#each selectedCase.topics as topic}{#if !topic.isActive && topic.role === 'primary'}<option value={topic.id} selected>{topic.name} (inactive)</option>{/if}{/each}
        </select></label>
    ...
    <div class="topic-secondary">
      ...
      {#if selectedCase.topics.filter((topic) => topic.role === 'secondary').length === 0}<p class="empty-state">No additional Study Topics are attached.</p>{:else}
        <div class="topic-list">
          {#each selectedCase.topics.filter((topic) => topic.role === 'secondary') as topic}
            <div class="topic-list-row">
              <div><a href={'/admin/topics/' + topic.id}>{topic.name}</a>{#if !topic.isActive}<span class="status-badge inactive">Inactive Topic — historical attachment</span>{/if}</div>
              <div class="actions">
                {#if topic.isActive}<form method="POST" action="?/promoteTopic"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="concept_id" value={topic.id} /><button class="button small" type="submit">Make primary</button></form>{/if}
                <form method="POST" action="?/removeSecondaryTopic"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="concept_id" value={topic.id} /><button class="button danger small" type="submit">Remove</button></form>
              </div>
            </div>
          {/each}
        </div>
      {/if}
      <form method="POST" action="?/addSecondaryTopic" class="topic-add-form">
        <input type="hidden" name="case_id" value={selectedCase.case.id} />
        <label>Add an active Study Topic<select name="concept_id" required>
          <option value="" disabled selected>Select a Topic</option>
          {#each data.concepts as concept}{#if !selectedCase.topics.some((topic) => topic.id === concept.id)}<option value={concept.id}>{concept.name}</option>{/if}{/each}
        </select></label>
  ...
  <section id="case" class="panel stack">
    ...
    <form method="POST" action="?/updateCase" class="form-grid">
      ...
      <label>Primary/default Topic<select name="concept_id" required>{#each data.concepts as concept}<option value={concept.id} selected={concept.id === selectedCase.case.conceptId}>{concept.name}</option>{/each}{#each selectedCase.topics as topic}{#if !topic.isActive && topic.role === 'primary'}<option value={topic.id} selected>{topic.name} (inactive)</option>{/if}{/each}</select></label>
  ...
  <section id="images" class="panel stack image-authoring">
    <div class="panel-heading">
      <div><p class="eyebrow">Clinical presentation</p><h2>Images</h2><p class="muted">Fixed images appear in every applicable Review. Each active alternative set selects one active image according to the existing stimulus rules.</p></div>
    ...
    <div class="image-subsection stack">
      ...
      {#each selectedCase.attached as asset, index}
        <article class="fixed-asset-card">
          ...
          {#if asset.imageUrl}
            <button class="fixed-image-preview" type="button" onclick={() => showImage(asset, 'Fixed Case image')} aria-label={`Enlarge ${asset.originalFilename ?? 'fixed image'}`}>
              <img src={asset.imageUrl} alt={asset.altText ?? ''} />
            </button>
          {:else}<div class="inactive-image large">Inactive image</div>{/if}
          <div class="asset-meta">
            {#if asset.sourceLabel}<span>Source: {#if asset.sourceUrl}<a href={asset.sourceUrl} target="_blank" rel="noreferrer">{asset.sourceLabel}</a>{:else}{asset.sourceLabel}{/if}</span>{/if}
            {#if asset.licence}<span>Licence: {asset.licence}</span>{/if}
          </div>
          ...
          <details class="advanced compact-disclosure">
            <summary>Alternative-set actions</summary>
            ...
            <div class="alternative-grid">
              {#each group.options as option, optionIndex}
                ...
                <article class="option-card">
                  ...
                  <details class="specific-questions" open={imageQuestions.length === 0}>
                    <summary>{imageQuestions.length} image-specific {imageQuestions.length === 1 ? 'question' : 'questions'}</summary>
                    <div class="specific-body stack">
                      <p class="muted">Only use these when relevance or the correct answer depends on this exact image.</p>
  ...
  {#if data.imagePicker?.open}
    <dialog bind:this={pickerDialog} class="image-picker" aria-labelledby="image-picker-heading">
      <div class="picker-shell">
        <div class="picker-heading"><div><p class="eyebrow">Asset Library</p><h2 id="image-picker-heading">{data.imagePicker.targetGroupName ? `Add images to ${data.imagePicker.targetGroupName}` : 'Add images from library'}</h2><p class="muted">Search is server-backed and limited to Assets not already used by this Case.</p></div><button bind:this={pickerCloseButton} class="button" type="button" onclick={() => pickerDialog?.close()}>Close</button></div>
        <form method="GET" action={`/admin/cases/${selectedCase.case.id}`} class="picker-search"><input type="hidden" name="picker" value="1" />{#if data.imagePicker.targetGroupId}<input type="hidden" name="target_group" value={data.imagePicker.targetGroupId} />{/if}<label>Search filename, alt text, or source<input name="image_q" value={data.imagePicker.search} placeholder="e.g. prolonged QTc" /></label><button class="button" type="submit">Search</button></form>
        {#if data.imagePicker.assets.length === 0}<p class="empty-state">No unused active images match this search.</p>{:else}
          <div class="picker-grid">
            {#each data.imagePicker.assets as asset}
              <label class:selected={pickerSelected.has(asset.id)} class="picker-card"><input type="checkbox" checked={pickerSelected.has(asset.id)} onchange={() => togglePickerAsset(asset.id)} /><img src={asset.imageUrl} alt={asset.altText ?? ''} loading="lazy" /><span><strong>{asset.originalFilename ?? asset.id}</strong><small>{asset.altText || 'No alt text'}</small>{#if asset.sourceLabel}<small>Source: {asset.sourceLabel}</small>{/if}</span></label>
            {/each}
          </div>
          {#if data.imagePicker.hasMore}<p class="picker-note">Showing the first {data.imagePicker.limit} matches. Refine the search to narrow the result set.</p>{/if}
        {/if}
