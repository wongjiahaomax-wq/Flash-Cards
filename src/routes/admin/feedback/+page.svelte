...
{#snippet reportRow(report)}
  <article class="report">
    ...
    <button class="report-open" type="button" onclick={() => openReport(report)} aria-label={'Open feedback report from ' + report.reporterLabel}>
...
{#if selectedReport}
  <div class="feedback-modal-shell" role="presentation" onclick={(event) => event.target === event.currentTarget && closeReport()}>
    <section class="feedback-modal" role="dialog" aria-modal="true" aria-labelledby="feedback-modal-title">
      <div class="modal-heading">
        <div>
          ...
          <h2 id="feedback-modal-title">{selectedReport.reporterLabel}</h2>
        </div>
        <button class="close-button" type="button" aria-label="Close feedback report" onclick={closeReport}>×</button>
      </div>
...
<style>
  ...
  .bulk-toolbar { display:none; align-items:center; justify-content:flex-end; gap:.75rem; margin:1rem 0; }
  ...
  .feedback-modal-shell { position:fixed; inset:0; z-index:60; display:grid; place-items:center; padding:1rem; background:rgb(23 32 51 / 35%); }
  .feedback-modal { width:min(100%, 560px); max-height:calc(100vh - 2rem); overflow:auto; padding:1.25rem; border:1px solid #cdd6e3; border-radius:14px; background:#fff; box-shadow:0 24px 60px rgb(23 32 51 / 22%); }
