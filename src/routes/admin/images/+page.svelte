...
<section class="page-heading">
  ...
  <div class="heading-actions"><button class:active={selectMode} class="button" type="button" onclick={() => (selectMode = !selectMode)}>{selectMode ? 'Done selecting' : 'Select images'}</button><a class="button primary" href="/admin/images/new">Upload image</a></div>
</section>
...
<section class="panel" aria-labelledby="image-list-heading">
  <div class="panel-heading"><div><h2 id="image-list-heading">Assets <span class="count">{data.assets.length}</span></h2><span class="muted">Click normally to edit. Ctrl/Cmd-click toggles; Shift-click selects a displayed range.</span></div>{#if selectedIds.size > 0}<button class="button small" type="button" onclick={resetSelection}>Clear selection</button>{/if}</div>
...
{#if selectedIds.size > 0}
  <form class="bulk-bar" method="POST" action="?/bulkAddToStimulusGroup" aria-label="Bulk image actions">
    ...
    <label>Alternative set<select name="group_id" required disabled={data.stimulusGroups.length === 0}><option value="">Choose Case — set</option>{#each data.stimulusGroups as group}<option value={group.id}>{group.caseTitle} — {group.name}</option>{/each}</select></label>
