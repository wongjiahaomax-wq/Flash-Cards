<script>
  ...
  let candidateById = $derived(new Map((candidateSet?.candidates ?? []).map((/** @type {DiscoveryCandidateRecord} */ candidate) => [candidate.id, candidate])));
