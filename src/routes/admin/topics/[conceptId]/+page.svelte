...
{:else}
  <section class="page-heading">
    <div>
      ...
      <p class="muted">A Topic holds knowledge that can be reused across several different clinical Cases.</p>
    </div>
    <div class="actions">
      <span class:inactive={!data.topic.isActive} class="status-badge">{data.topic.isActive ? 'Active' : 'Inactive'}</span>
      <a class="button primary" href={'/admin/cases/new?concept=' + data.topic.id}>Add Case in this Topic</a>
    </div>
  </section>

  <section class="panel identity">
    <div><h2>Topic identity</h2>{#if data.topic.descriptionMd}<p class="description">{data.topic.descriptionMd}</p>{:else}<p class="muted">No description.</p>{/if}<p class="muted technical">Internal slug: <code>{data.topic.slug}</code></p></div>
    <dl>
      <div><dt>Parent Topic</dt><dd>{#if data.topic.parent}<a href={'/admin/topics/' + data.topic.parent.id}>{data.topic.parent.name}</a>{:else}—{/if}</dd></div>
  ...
  <section class="panel">
    <div class="panel-heading"><div><h2>Cases <span class="count">{data.topic.cases.length}</span></h2><p class="muted section-copy">Different stems or clinical presentations should be separate Cases even when they teach the same Topic.</p></div><span class="muted">Primary Topic relationships</span></div>
    {#if data.topic.cases.length === 0}<p class="empty-state">No Cases use this as their primary Topic yet.</p>{:else}
  ...
  <section class="panel">
    <div class="panel-heading"><div><h2>Shared Topic Questions <span class="count">{data.topic.questions.length}</span></h2><p class="muted section-copy">Use this level when the answer remains correct across compatible Cases in this Topic.</p></div><span class="muted">Reusable knowledge</span></div>
    {#if data.topic.questions.length === 0}<p class="empty-state">No reusable questions are attached directly to this Topic.</p>{:else}
      <div class="question-list">{#each data.topic.questions as question}
        <article class="question-card">
          ...
          <p class="meta">Available to descendant Topics: <strong>{question.inheritToDescendants ? 'Yes' : 'No'}</strong></p>
        </article>
      {/each}</div>
    {/if}
  </section>
...
<style>
  .page-heading,.panel-heading,.question-heading,.row,.actions { display:flex; justify-content:space-between; align-items:center; gap:1rem; }
  ...
  .eyebrow { margin-bottom:.3rem; color:#667085; font-size:.74rem; font-weight:750; letter-spacing:.08em; text-transform:uppercase; } .muted,.meta { color:#667085; } code { font-size:.9em; } .technical { margin:.8rem 0 0; font-size:.86rem; } .section-copy { margin:.35rem 0 0; max-width:48rem; }
  ...
  .button { display:inline-block; padding:.65rem .85rem; border:1px solid #cdd6e3; border-radius:8px; color:#172033; text-decoration:none; white-space:nowrap; } .button.primary { border-color:#172033; background:#172033; color:#fff; }
  .empty-state { margin:1rem 0 0; padding:1rem; border:1px dashed #d0d5dd; border-radius:8px; } .description { white-space:pre-wrap; }
  @media (max-width:680px) { .identity { grid-template-columns:1fr; } .page-heading,.panel-heading { align-items:start; flex-direction:column; } .actions { align-items:flex-start; flex-wrap:wrap; } }
</style>
