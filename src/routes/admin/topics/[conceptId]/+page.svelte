...
{:else}
  ...
  <section class="panel metrics">
    <div><span>Direct primary Cases</span><strong>{data.topic.kind === 'topic' ? data.topic.directCaseCount : '—'}</strong></div>
  ...
  {:else}
    <section class="panel">
      <div class="panel-heading"><div><h2>Cases <span class="count">{data.topic.cases.length}</span></h2><p class="muted section-copy">Direct primary relationships only. Additional Study Topic routes remain on each Case.</p></div></div>
      {#if data.topic.cases.length === 0}<p class="empty-state">No Cases use this as their primary Topic yet.</p>{:else}<div class="stack">{#each data.topic.cases as item}<a class="row" href={'/admin/cases/' + item.caseId}><strong>{item.caseTitle}</strong><span class:inactive={!item.caseIsActive} class="status-badge">{item.caseIsActive ? 'Active' : 'Inactive'}</span></a>{/each}</div>{/if}
    </section>
...
<style>
  ...
  .count { color:#667085; font-size:.85rem; font-weight:500; } .stack,.question-list { display:grid; gap:.65rem; margin-top:1rem; } .row { padding:.75rem; border:1px solid #eaecf0; border-radius:8px; color:#172033; text-decoration:none; } .row > span:first-child { display:grid; gap:.15rem; } .row small,.row-meta { color:#667085; } .route-chips { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:.35rem; } .route-chips em { padding:.15rem .4rem; border-radius:999px; background:#f2f4f7; color:#475467; font-size:.76rem; font-style:normal; }
