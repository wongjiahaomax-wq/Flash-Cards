<script>
  ...
  function imagePickerHref(targetGroupId = '') {
    const params = new URLSearchParams({ picker: '1' });
    if (targetGroupId) params.set('target_group', targetGroupId);
...
<section id="images" class="panel stack image-authoring">
  <div class="panel-heading"><div><p class="eyebrow">Clinical presentation</p><h2>Images{#if editorLayout === 'compact'}<AccessibleInfo label="Case images" text="Always-shown Case images remain visible whenever this Case is reviewed. Each active image set supplies one learner-visible image according to the existing Original/Alternative rules." />{/if}</h2><p class="muted compact-hide-explainer">Always-shown Case images remain visible in each applicable Review. Image sets contain the Original and any interchangeable Alternatives.</p></div><a class="button primary" href={imagePickerHref()}>Add images from library</a></div>
  <div class="image-subsection stack">
    <div><h3>Always-shown Case images <span class="count">{selectedCase.attached.length}</span>{#if editorLayout === 'compact'}<AccessibleInfo label="Always-shown Case image" text="This image remains visible alongside any Original or Alternative selected from an image set." />{/if}</h3><p class="muted compact-hide-explainer">These Case images remain visible alongside the selected image-set stimulus. You can edit their captions and explicitly reuse canonical Asset questions.</p></div>
    {#if editorLayout === 'compact' && selectedCase.attached.length > 1}
      <CaseImageStrip label="Always-shown Case images" items={selectedCase.attached.map((asset) => ({ ...asset, id: `fixed-${asset.assetId}`, status: 'ALWAYS SHOWN' }))} onselect={(item) => focusReviewTarget(`fixed-image-${item.assetId}`)} />
    {/if}
    {#if selectedCase.attached.length === 0}<p class="empty-state">No always-shown Case images attached yet.</p>{/if}
    {#each selectedCase.attached as asset, index}
      ...
      <article id={`fixed-image-${asset.assetId}`} class="fixed-asset-card" tabindex="-1">
        <div class="asset-card-heading"><div class="asset-title"><span class="order-badge">{index + 1}</span><div><strong>{asset.originalFilename ?? asset.assetId}</strong><span class="muted">{asset.altText || 'No alt text'}</span></div></div><span class="status-badge">Always shown</span></div>
        {#if asset.imageUrl}<button class="fixed-image-preview" type="button" onclick={() => showImage(asset, 'Always-shown Case image')} aria-label={`Enlarge ${asset.originalFilename ?? 'Case image'}`}><img src={asset.imageUrl} alt={asset.altText ?? ''} /></button>{:else}<div class="inactive-image large">Inactive image</div>{/if}
        ...
        <form method="POST" action="?/caption" class="stack" data-case-editor-coordinated use:coordinateForm={`caption:${asset.assetId}`}><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="asset_id" value={asset.assetId} /><MarkdownField label="Case-specific caption" id={`fixed-caption-${asset.assetId}`} name="caption" value={asset.captionMd ?? ''} rows={2} maxlength={1000} /><button class="button small" type="submit">Save caption</button></form>
        ...
        {#if !previewMode && asset.isActive}
          <details class="specific-questions"><summary>Manage questions</summary><div class="specific-body stack">
            <section class="question-management-section"><div><h4>Case-specific Image Questions <span class="count">0</span></h4><p class="muted">These belong only to this Case + image context. Adding one preserves the exact-image relationship while using the existing safe image-set conversion.</p></div>
              <form method="POST" action={`/admin/cases/${selectedCase.case.id}/question-scope`} class="stack image-question-form"><input type="hidden" name="return_query" value={caseLibraryReturnQuery} /><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="scope" value="stimulus" /><input type="hidden" name="target" value={`fixed:${asset.assetId}`} /><div class="question-prompt-field"><MarkdownField label="Question prompt" id={`fixed-image-question-prompt-${asset.assetId}`} name="prompt_md" rows={2} required maxlength={2000} placeholder="e.g. What are the ECG changes?" textareaAction={autoGrowImageField} /></div><div class="question-answer-field"><MarkdownField label="Answer for this Case and image" id={`fixed-image-question-answer-${asset.assetId}`} name="answer_md" rows={2} required maxlength={10000} textareaAction={autoGrowImageField} /></div><div class="image-question-actions"><button class="button small" type="submit">Add Case-specific Image Question</button></div></form>
              {#if selectedCase.questions.length > 0}<details class="move-existing-question"><summary>Move existing Case question here</summary><form method="POST" action={`/admin/cases/${selectedCase.case.id}/question-scope`} class="specific-body stack"><input type="hidden" name="return_query" value={caseLibraryReturnQuery} /><input type="hidden" name="intent" value="move" /><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="target" value={`fixed:${asset.assetId}`} /><label>Case question<select name="prompt_id" required><option value="" disabled selected>Choose a Case question</option>{#each selectedCase.questions as caseQuestion}<option value={caseQuestion.questionPromptId}>{caseQuestion.promptMd}</option>{/each}</select></label><div><button class="button small" type="submit">Move question here</button></div></form></details>{/if}
            </section>
        <ReusableImageQuestionManager summary={reusable} caseId={selectedCase.case.id} assetId={asset.assetId} previewMode={previewMode} {coordinator} />
          </div></details>
        {/if}
        ...
        <details class="advanced compact-disclosure"><summary>Image-set actions</summary><div class="advanced-body stack"><form method="POST" action="?/startAlternativeSet" class="move-to-alternatives"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="asset_id" value={asset.assetId} /><label>New image-set name<input name="set_name" required placeholder="e.g. ECG" /></label><button class="button small" type="submit">Start image set with this Original</button></form>{#if selectedCase.stimulusGroups.length > 0}<form method="POST" action="?/addStimulusOption" class="move-to-alternatives"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="asset_id" value={asset.assetId} /><input type="hidden" name="convert_fixed" value="on" /><label>Add to image set<select name="group_id" required><option value="">Choose image set</option>{#each selectedCase.stimulusGroups as group}<option value={group.id}>{group.name}</option>{/each}</select></label><button class="button small" type="submit">Add as Alternative</button></form>{/if}</div></details>
      </article>
    {/each}
  </div>

  <div class="image-subsection stack">
    <div class="subsection-heading"><div><h3>Image sets <span class="count">{selectedCase.stimulusGroups.length}</span>{#if editorLayout === 'compact'}<AccessibleInfo label="Image set" text="An image set groups one Original with interchangeable Alternatives for the same Case." />{/if}</h3><p class="muted compact-hide-explainer">Use an image set when the Case stays the same but the learner may see a different interchangeable example image.</p></div></div>
    <details class="create-set-disclosure"><summary>Create an empty image set</summary><div class="advanced-body"><form method="POST" action="?/createStimulusGroup" class="start-alternative-form"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="specific_question_mode" value="none" /><label>Image-set name<input name="name" required placeholder="e.g. ECG" /></label><button class="button" type="submit">Create image set</button></form></div></details>
    {#if selectedCase.stimulusGroups.length === 0}<p class="empty-state">No image sets yet. Start one from a Case image when you need an Original with interchangeable Alternatives.</p>{/if}
    {#each selectedCase.stimulusGroups as group}
      <article class="alternative-set">
        <div class="card-heading"><div><p class="eyebrow">Image set</p><h3>{group.name} <span class="count">{group.options.length} images</span></h3></div><div class="actions"><span class:inactive={!group.isActive} class="status-badge">{group.isActive ? 'Active' : 'Inactive'}</span>{#if group.isActive}<a class="button small" href={imagePickerHref(group.id)}>+ Add image</a>{/if}</div></div>
        {#if editorLayout === 'compact' && group.options.length > 0}
          <CaseImageStrip label={`${group.name} image set`} items={group.options.map((option) => ({ ...option, status: option.isActive && option.assetIsActive ? `IMAGE SET · ${group.name}` : `INACTIVE · ${group.name}` }))} onselect={(item) => focusReviewTarget(`option-review-${item.id}`)} />
        {/if}
          ...
          <div class="alternative-grid">
            {#each group.options as option, optionIndex}
              ...
              <article class="option-card" class:selected={selectedOptionId === option.id}>
                ...
                {#if option.imageUrl}<button class="option-thumbnail" type="button" onclick={() => showImage(option, `${group.name} image set`)} aria-label={`Enlarge ${option.originalFilename ?? 'Case image'}`}><img src={option.imageUrl} alt={option.altText ?? 'Case image'} loading="lazy" /></button>{:else}<div class="inactive-image option-thumb">Inactive image</div>{/if}
            ...
            {#if option}
              ...
              <section id={`option-editor-${option.id}`} class="option-editor" aria-labelledby={`option-editor-heading-${option.id}`} tabindex="-1">
                ...
                <div class="option-editor-layout">
                  <div class="option-editor-preview">{#if option.imageUrl}<button class="option-thumbnail" type="button" onclick={() => showImage(option, `${group.name} image set`)} aria-label={`Enlarge ${option.originalFilename ?? 'Case image'}`}><img src={option.imageUrl} alt={option.altText ?? 'Case image'} /></button>{:else}<div class="inactive-image option-thumb">Inactive image</div>{/if}<span class:inactive={!option.isActive || !option.assetIsActive} class="status-badge">{option.isActive && option.assetIsActive ? 'Active' : 'Inactive'}</span></div>
                  <div class="option-editor-fields stack">
                    <form method="POST" action="?/updateStimulusOptionCaption" class="specific-body stack" data-case-editor-coordinated use:coordinateForm={`option-caption:${option.id}`}><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="option_id" value={option.id} /><MarkdownField label="Case-specific caption" id={`option-caption-${option.id}`} name="caption" value={option.captionMd ?? ''} rows={3} maxlength={1000} /><div><button class="button small" type="submit">Save caption</button></div></form>
                    {#if option.isActive && group.isActive && selectedCase.stimulusGroups.some((candidate) => candidate.id !== group.id && candidate.isActive)}
                      <details class="option-move"><summary>Move to another image set…</summary><form method="POST" action={`${editorBase}/cases/${selectedCase.case.id}/move-option`} class="specific-body stack"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="option_id" value={option.id} /><label>Target image set<select name="target_group_id" required><option value="" disabled selected>Choose an image set</option>{#each selectedCase.stimulusGroups as candidate}{#if candidate.id !== group.id && candidate.isActive}<option value={candidate.id}>{candidate.name}</option>{/if}{/each}</select></label><p class="muted">The option identity, Case-specific caption and exact-image questions stay attached. Shared image-set questions stay with their current image sets.</p><div><button class="button small" type="submit">Move image</button></div></form></details>
                    {/if}
                    <section class="specific-questions option-editor-questions" aria-labelledby={`questions-${option.id}`}><div class="section-heading"><h4 id={`questions-${option.id}`}>Case-specific Image Questions <span class="count">{imageQuestions.length}</span></h4><p class="muted">These belong only to this Case + exact image context.</p></div><form method="POST" action="?/saveStimulusOptionQuestion" class="stack image-question-form"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="option_id" value={option.id} /><div class="question-prompt-field"><MarkdownField label="Question prompt" id={`option-image-question-prompt-${option.id}`} name="prompt_md" rows={3} required maxlength={2000} placeholder="e.g. Describe this ECG." textareaAction={autoGrowImageField} /></div><div class="question-answer-field"><MarkdownField label="Answer for this Case and image" id={`option-image-question-answer-${option.id}`} name="answer_md" rows={3} required maxlength={10000} textareaAction={autoGrowImageField} /></div><div class="image-question-actions"><button class="button small" type="submit">Add Case-specific Image Question</button></div></form>{#if !previewMode && selectedCase.questions.length > 0}<details class="move-existing-question"><summary>Move existing Case question here</summary><div class="specific-body stack"><p class="muted">Move a question from this Case to this exact image. Its prompt and answer will be preserved.</p><form method="POST" action={`/admin/cases/${selectedCase.case.id}/question-scope`} class="stack"><input type="hidden" name="return_query" value={caseLibraryReturnQuery} /><input type="hidden" name="intent" value="move" /><input type="hidden" name="case_id" value={selectedCase.case.id} /><label>Case question<select name="prompt_id" required><option value="" disabled selected>Choose a Case question</option>{#each selectedCase.questions as caseQuestion}<option value={caseQuestion.questionPromptId}>{caseQuestion.promptMd}</option>{/each}</select></label><div><button class="button small" type="submit">Move question here</button></div></form></div></details>{/if}{#each imageQuestions as question}<div class="question-card compact"><form method="POST" action="?/saveStimulusOptionQuestion" class="stack image-question-form" data-case-editor-coordinated use:coordinateForm={`option-question:${option.id}:${question.id}`}><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="option_id" value={option.id} /><input type="hidden" name="stimulus_question_id" value={question.id} /><input type="hidden" name="original_prompt_id" value={question.questionPromptId} /><div class="question-prompt-field"><MarkdownField label="Question prompt" id={`option-image-question-prompt-${question.id}`} name="prompt_md" value={question.promptMd} rows={2} required maxlength={2000} textareaAction={autoGrowImageField} /></div><div class="question-answer-field"><MarkdownField label="Answer for this Case and image" id={`option-image-question-answer-${question.id}`} name="answer_md" value={question.answerMd} rows={2} required maxlength={10000} textareaAction={autoGrowImageField} /></div><div class="image-question-actions"><button class="button small" type="submit">Save Case-specific Image Question</button></div></form><form method="POST" action="?/removeStimulusQuestion"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="scope" value="option" /><input type="hidden" name="context_id" value={option.id} /><input type="hidden" name="prompt_id" value={question.questionPromptId} /><button class="button danger small" type="submit">Remove</button></form></div>{/each}</section>
                  <ReusableImageQuestionManager summary={reusable} caseId={selectedCase.case.id} assetId={option.assetId} optionId={option.id} previewMode={previewMode} {coordinator} />
                  </div>
                </div>
              </section>
            {/if}
        ...
        {#if editorLayout === 'compact' && group.isActive && group.options.some((option) => option.isActive && option.assetIsActive) && group.questions.some((question) => question.isActive)}
          <section id={`set-wide-${group.id}`} class="compact-set-wide-review" tabindex="-1">
            <div class="scope-review-heading"><strong>SHARED ACROSS IMAGE SET · {group.name}</strong><AccessibleInfo label="Shared image-set question" text="This Prompt/Answer pair applies to every active image option in this image set. It is not attached to one exact image." /></div>
            {#each group.questions.filter((question) => question.isActive) as question (question.id)}
              <form method="POST" action="?/saveStimulusGroupQuestion" class="compact-set-wide-row" data-case-editor-coordinated use:coordinateForm={`group-question:${group.id}:${question.id}`}><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="group_id" value={group.id} /><input type="hidden" name="stimulus_question_id" value={question.id} /><input type="hidden" name="original_prompt_id" value={question.questionPromptId} /><MarkdownField label="Prompt" id={`group-question-prompt-${question.id}`} name="prompt_md" value={question.promptMd} rows={2} required maxlength={2000} /><MarkdownField label="Answer" id={`group-question-answer-${question.id}`} name="answer_md" value={question.answerMd} rows={2} required maxlength={10000} textareaAction={autoGrowImageField} /><div><button class="button small" type="submit">Save</button></div></form>
            {/each}
          </section>
        {/if}
        <details class="advanced"><summary>Advanced image-set settings</summary><div class="advanced-body stack"><p class="muted">Coverage controls how strongly questions specific to this image set are represented in a Review.</p><form method="POST" action="?/updateStimulusGroup" class="form-grid" data-case-editor-coordinated use:coordinateForm={`group-settings:${group.id}`}><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="group_id" value={group.id} /><label>Image-set name<input name="name" value={group.name} required /></label><label>Specific-question coverage<select name="specific_question_mode"><option value="none" selected={group.specificQuestionMode === 'none'}>No guarantee</option><option value="minimum" selected={group.specificQuestionMode === 'minimum'}>At least N</option><option value="all" selected={group.specificQuestionMode === 'all'}>All available</option></select></label><label>Minimum specific questions<input type="number" name="minimum_specific_questions" min="1" value={group.minimumSpecificQuestions ?? ''} /></label><label class="checkbox-label"><input type="checkbox" name="is_active" checked={group.isActive} /> Active image set</label><div><button class="button" type="submit">Save image-set settings</button></div></form></div></details>
        <details class="advanced"><summary>{group.questions.length} shared image-set {group.questions.length === 1 ? 'question' : 'questions'} — advanced</summary><div class="advanced-body stack"><p class="muted">Use this only when a prompt and answer are valid for every image in this image set but should not be a general Topic or Case question.</p><form method="POST" action="?/saveStimulusGroupQuestion" class="form-grid"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="group_id" value={group.id} /><MarkdownField label="Question prompt" id={`group-new-question-prompt-${group.id}`} name="prompt_md" rows={2} required maxlength={2000} /><MarkdownField label="Answer" id={`group-new-question-answer-${group.id}`} name="answer_md" rows={2} required maxlength={10000} textareaAction={autoGrowImageField} /><div><button class="button" type="submit">Add shared image-set question</button></div></form>{#each group.questions as question}<div class="question-card compact"><strong>{question.promptMd}</strong><span>{question.answerMd}</span><form method="POST" action="?/removeStimulusQuestion"><input type="hidden" name="case_id" value={selectedCase.case.id} /><input type="hidden" name="scope" value="group" /><input type="hidden" name="context_id" value={group.id} /><input type="hidden" name="prompt_id" value={question.questionPromptId} /><button class="button danger small" type="submit">Remove</button></form></div>{/each}</div></details>
      </article>
    {/each}
  </div>
</section>
