<script>
  ...
  let { selectedCase, previewMode, studyPreviewHref = null, caseLibraryReturnQuery = '', coordinator = null, draftRevision = 0, feedback = null, feedbackReturnQuery = '', onfeedbackopen = () => {}, ondiscardall = () => {} } = $props();
...
<section class="page-heading">
  ...
  </div></details>{#if saveableCount}<button class="button primary save-all-button" type="button" onclick={saveAll} disabled={coordinator.isSavingAll()} aria-label="Save all saveable Case-editor changes">{coordinator.isSavingAll() ? 'Saving…' : 'Save all changes'}</button>{/if}<button class="button danger-outline" type="button" onclick={() => ondiscardall()} disabled={coordinator.isSavingAll()} aria-label="Discard all unsaved Case-editor changes">Discard all changes</button>{/if}{#if structuralItems.length}<span class="save-all-guidance">Structural work remains Not submitted and is not included in Save All.</span>{/if}{#if saveAllResult?.failed && saveAllResult.attempted}<span class="save-all-result error" role="alert">Save All did not complete. Some changes may already have been saved; captured drafts remain marked unsaved. Review them before retrying.</span>{:else if saveAllResult?.failed}<span class="save-all-result error" role="alert">Save All could not start; unsaved changes remain.</span>{:else if saveAllResult?.attempted}<span class="save-all-result" role="status">{saveAllResult.succeeded} saved</span>{/if}{#if previewMode}<span class="muted">Learner Study is unavailable in Preview Mode.</span>{:else}<a class="button primary" href={studyPreviewHref ?? '/study'}>Preview in Study</a>{/if}</div>
</section>
...
<style>
  ...
  .button.danger-outline { border-color: #fecdca; color: #b42318; }
  .feedback-button { color: #475467; }
  .feedback-count { display: inline-grid; min-width: 1.3rem; height: 1.3rem; margin-left: .35rem; padding: 0 .25rem; place-items: center; border-radius: 999px; background: #eef2f6; color: #344054; font-size: .75rem; font-weight: 750; }
