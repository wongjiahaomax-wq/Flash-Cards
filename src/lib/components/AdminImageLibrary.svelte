<script>
  /** @typedef {{ id: string, imageUrl: string | null, altText: string | null, originalFilename: string | null, topicSummary: string, topicNames: string[], collectionName: string | null, createdAt: string | number | Date | null, usageCount: number, isActive: boolean }} LibraryAsset */
...
{#if selectedIds.size > 0}
  <div class="bulk-bar" aria-label="Bulk image actions">
    ...
    {#if !previewMode}<label>Collection<select bind:value={bulkCollectionId} required disabled={bulkRunning}><option value="">Choose Collection action</option><option value="__unsorted__">Unsorted (remove Collection)</option>{#each data.collections as collection}<option value={collection.id}>{collection.name}</option>{/each}</select></label><button class="button primary" type="button" onclick={runBulkCollection} disabled={!bulkCollectionId || bulkRunning}>{bulkRunning ? 'Updating…' : 'Set Collection'}</button>{/if}
...
<style>
  ...
  .collection-create,.collection-management{margin:1rem 0}.collection-create{display:flex;justify-content:space-between;align-items:center;gap:1rem}.collection-create form{display:flex;gap:.5rem;align-items:end}.collection-create input{min-width:180px}.collection-list{display:grid;gap:.5rem;margin-top:1rem}.collection-row{display:grid;grid-template-columns:minmax(160px,1fr) minmax(260px,1.4fr) auto;align-items:center;gap:.75rem;padding:.65rem 0;border-top:1px solid #eaecf0}.collection-row>div{display:grid;gap:.2rem}.collection-rename-form{display:flex;gap:.5rem}.button.danger{border-color:#fecdca;color:#b42318}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.selection-summary{display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;margin-top:1rem;padding:.75rem;border-radius:8px;background:#f8fafc}.asset-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:1rem;margin-top:1rem}.asset-card-wrap{position:relative;display:grid;overflow:hidden;border:2px solid transparent;border-radius:12px}.asset-card-wrap.selected{border-color:#344054}.selection-box{position:absolute;z-index:2;top:.55rem;left:.55rem;padding:.35rem;border-radius:7px;background:rgb(255 255 255 / 92%);cursor:pointer}.selection-box input{width:1.1rem;height:1.1rem;margin:0;accent-color:#172033}.asset-card{display:grid;overflow:hidden;border:1px solid #dfe5ee;border-radius:10px 10px 0 0;background:#fff;color:#172033;text-decoration:none}.asset-card img,.inactive-thumb{width:100%;height:150px;object-fit:contain;background:#eef2f6}.inactive-thumb{display:grid;place-items:center;color:#667085;font-size:.85rem}.asset-card-body{display:grid;gap:.3rem;padding:.8rem;overflow-wrap:anywhere}.collection-context{color:#175cd3;font-size:.82rem;font-weight:700}.topic-context{color:#344054;font-size:.82rem;font-weight:650}.status-badge{width:max-content;padding:.18rem .45rem;border-radius:999px;font-size:.72rem;font-weight:700}.badge-active{background:#ecfdf3;color:#027a48}.badge-inactive{background:#f2f4f7;color:#667085}.enlarge-button{padding:.5rem;border:1px solid #dfe5ee;border-top:0;border-radius:0 0 10px 10px;background:#f8fafc;color:#344054;font:inherit;font-size:.82rem;font-weight:650;cursor:zoom-in}.empty-state{margin-top:1rem;padding:1rem;border:1px dashed #d0d5dd;border-radius:8px}
  ...
  @media(max-width:1200px){.filter-panel{grid-template-columns:repeat(3,minmax(0,1fr))}.search-field{grid-column:1/-1}.bulk-bar{grid-template-columns:1fr 1fr}}@media(max-width:700px){.page-heading,.panel-heading{align-items:start;flex-direction:column}.filter-panel{grid-template-columns:repeat(2,minmax(0,1fr))}.search-field,.filter-actions{grid-column:1/-1}.collection-create{align-items:stretch;flex-direction:column}.collection-create form,.collection-rename-form{align-items:stretch;flex-direction:column}.collection-create input{min-width:0}.collection-row{grid-template-columns:1fr}.asset-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.bulk-bar{grid-template-columns:1fr}.pagination{grid-template-columns:auto 1fr auto;font-size:.85rem}}@media(max-width:430px){.asset-grid{grid-template-columns:1fr}}
</style>
