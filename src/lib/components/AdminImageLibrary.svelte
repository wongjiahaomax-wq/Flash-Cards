...
{#if form?.cleanupRetried}<p class="form-success" role="status">{form.cleanupMessage}</p>{/if}
...
{#if !previewMode && data.pendingCleanup?.length}
  <section class="panel cleanup-pending" aria-labelledby="dedupe-cleanup-heading">
...
<section class="panel" aria-labelledby="image-list-heading">
  <div class="panel-heading"><div><h2 id="image-list-heading">Assets <span class="count">{data.pagination.totalCount}</span></h2><span class="muted">Showing {firstShown}–{lastShown} of {data.pagination.totalCount} images · Page {data.pagination.page} of {data.pagination.totalPages}. Shift ranges stay on the current page.</span></div>{#if selectedIds.size > 0}<div class="heading-actions"><button class="button small" type="button" onclick={resetSelection}>Clear selection</button></div>{/if}</div>

  {#if !previewMode}
    <div class="discovery-callout">
      <div class="discovery-copy"><strong>Looking for duplicate images?</strong><span class="muted">Run a read-only visual search over a Topic, a System, or the whole library. It proposes likely and possible duplicate pairs for you to compare and certify — nothing is merged automatically.</span></div>
      <a class="button primary" href="/admin/images/duplicates">Find visual duplicates</a>
    </div>
  {/if}
...
<style>
  ...
  .preview-collection-note,.collection-create,.collection-management,.cleanup-pending{margin:1rem 0}.preview-collection-note{display:flex;align-items:baseline;gap:.4rem;flex-wrap:wrap}.collection-create{display:flex;justify-content:space-between;align-items:center;gap:1rem}.collection-create form{display:flex;gap:.5rem;align-items:end}.collection-create input{min-width:180px}.collection-list,.cleanup-list{display:grid;gap:.5rem;margin-top:1rem}.collection-row{display:grid;grid-template-columns:minmax(160px,1fr) minmax(260px,1.4fr) auto;align-items:center;gap:.75rem;padding:.65rem 0;border-top:1px solid #eaecf0}.collection-row>div,.cleanup-row>div{display:grid;gap:.2rem}.collection-rename-form{display:flex;gap:.5rem}.cleanup-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:.75rem;padding:.65rem 0;border-top:1px solid #eaecf0}.button.danger{border-color:#fecdca;color:#b42318}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.selection-summary{display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;margin-top:1rem;padding:.75rem;border-radius:8px;background:#f8fafc}.discovery-callout{display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-top:1rem;padding:.9rem 1rem;border:1px solid #b2ccff;border-left:4px solid #175cd3;border-radius:10px;background:#f5f8ff}.discovery-copy{display:grid;gap:.15rem;min-width:0}.discovery-callout .button.primary{white-space:nowrap}.asset-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:1rem;margin-top:1rem}.asset-card-wrap{position:relative;display:grid;overflow:hidden;border:2px solid transparent;border-radius:12px}.asset-card-wrap.selected{border-color:#344054}.selection-box{position:absolute;z-index:2;top:.55rem;left:.55rem;padding:.35rem;border-radius:7px;background:rgb(255 255 255 / 92%);cursor:pointer}.selection-box input{width:1.1rem;height:1.1rem;margin:0;accent-color:#172033}.asset-card{display:grid;overflow:hidden;border:1px solid #dfe5ee;border-radius:10px 10px 0 0;background:#fff;color:#172033;text-decoration:none}.asset-card img,.inactive-thumb{width:100%;height:150px;object-fit:contain;background:#eef2f6}.inactive-thumb{display:grid;place-items:center;color:#667085;font-size:.85rem}.asset-card-body{display:grid;gap:.3rem;padding:.8rem;overflow-wrap:anywhere}.collection-context{color:#175cd3;font-size:.82rem;font-weight:700}.topic-context{color:#344054;font-size:.82rem;font-weight:650}.topic-context-historical{color:#667085;font-weight:600}.status-badge{width:max-content;padding:.18rem .45rem;border-radius:999px;font-size:.72rem;font-weight:700}.badge-active{background:#ecfdf3;color:#027a48}.badge-inactive{background:#f2f4f7;color:#667085}.enlarge-button{padding:.5rem;border:1px solid #dfe5ee;border-top:0;border-radius:0 0 10px 10px;background:#f8fafc;color:#344054;font:inherit;font-size:.82rem;font-weight:650;cursor:zoom-in}.empty-state{margin-top:1rem;padding:1rem;border:1px dashed #d0d5dd;border-radius:8px}
  ...
  @media(max-width:1200px){.filter-panel{grid-template-columns:repeat(3,minmax(0,1fr))}.search-field{grid-column:1/-1}.bulk-bar{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}}@media(max-width:700px){.page-heading,.panel-heading{align-items:start;flex-direction:column}.discovery-callout{align-items:stretch;flex-direction:column}.filter-panel{grid-template-columns:repeat(2,minmax(0,1fr))}.search-field,.filter-actions{grid-column:1/-1}.collection-create{align-items:stretch;flex-direction:column}.collection-create form,.collection-rename-form{align-items:stretch;flex-direction:column}.collection-create input{min-width:0}.collection-row,.cleanup-row{grid-template-columns:1fr}.asset-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.bulk-bar{grid-template-columns:minmax(0,1fr)}.pagination{grid-template-columns:auto 1fr auto;font-size:.85rem}}@media(max-width:430px){.asset-grid{grid-template-columns:1fr}}
</style>
