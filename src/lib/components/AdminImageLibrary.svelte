<script>
  /** @typedef {{ id: string, imageUrl: string | null, altText: string | null, originalFilename: string | null, topicSummary: string, topicNames: string[], collectionName: string | null, createdAt: string | number | Date | null, usageCount: number, usageState: 'current' | 'historical' | 'unused', historicalReviewCount: number, isActive: boolean }} LibraryAsset */
...
<nav class="cleanup-shortcuts" aria-label="Image cleanup views"><strong>Cleanup:</strong><a href={`${libraryPath}?usage=historical&sort=oldest`}>Historical only</a><a href={`${libraryPath}?usage=unused&sort=oldest`}>Unused</a><span class="muted">Review retained images or start with the oldest cleanup candidates.</span></nav>
      ...
      {#each data.assets as asset}
        <div class:selected={selectedIds.has(asset.id)} class="asset-card-wrap">
          ...
          <a class="asset-card" href={previewMode ? libraryPath : `/admin/images/${asset.id}`} onclick={(event) => handleCardClick(event, asset.id)} aria-label={`${asset.originalFilename ?? 'Unnamed image'}; ${selectedIds.has(asset.id) ? 'selected' : 'not selected'}`}>
            ...
            <div class="asset-card-body"><strong>{asset.originalFilename ?? 'Unnamed image'}</strong><span class="collection-context">Collection: {asset.collectionName ?? 'Unsorted'}</span>{#if asset.topicSummary}<span class="topic-context" title={asset.topicNames.join(', ')}>Used in Topics: {asset.topicSummary}</span>{/if}<span class="muted">Added {formatAddedDate(asset.createdAt)}</span><span class:usage-current={asset.usageState === 'current'} class:usage-historical={asset.usageState === 'historical'} class:usage-unused={asset.usageState === 'unused'} class="usage-badge">{#if asset.usageState === 'current'}Current · {asset.usageCount} {asset.usageCount === 1 ? 'Case' : 'Cases'}{:else if asset.usageState === 'historical'}Historical only{#if asset.historicalReviewCount > 0} · {asset.historicalReviewCount} past {asset.historicalReviewCount === 1 ? 'Review' : 'Reviews'}{/if}{:else}Unused{/if}</span><span class:badge-active={asset.isActive} class:badge-inactive={!asset.isActive} class="status-badge">{asset.isActive ? 'Active' : 'Inactive'}</span></div>
          </a>
