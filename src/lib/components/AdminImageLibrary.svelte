...
<style>
  ...
  .pagination{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:1rem;margin-top:1rem}.pagination>a:last-child{justify-self:end}.bulk-bar{position:sticky;z-index:10;bottom:.75rem;display:grid;grid-template-columns:minmax(190px,auto) minmax(240px,1fr) auto minmax(240px,1fr) auto auto;align-items:end;gap:.75rem;margin:1rem auto 0;padding:.85rem;border:1px solid #98a2b3;border-radius:10px;background:#fff;box-shadow:0 10px 35px rgb(16 24 40 / 18%)}.bulk-bar>div{display:grid;gap:.15rem}.bulk-bar>div span,.bulk-note{color:#667085;font-size:.78rem}.bulk-note,progress{grid-column:1/-1}progress{width:100%}
