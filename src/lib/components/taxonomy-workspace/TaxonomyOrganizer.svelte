<script lang="ts">
  import CaseClassificationInspector from './CaseClassificationInspector.svelte';
  import SearchableTaxonomyPicker from './SearchableTaxonomyPicker.svelte';
  import StagedTaxonomyReview from './StagedTaxonomyReview.svelte';
  import TaxonomyWorkspaceInspector from './TaxonomyWorkspaceInspector.svelte';
  ...
  import { stageFlexibleCasePrimaryTopicChanges } from './case-primary-topic-workspace-model.ts';
  import {
    activeSystemOptions,
  ...
  let draggedCaseIds = $state<string[]>([]);
  ...
  function toggleCaseSelection(caseId: string) {
    selectedCaseIds = toggleId(selectedCaseIds, caseId).slice(0, 60);
    selectedId = '';
    workspaceError = '';
  }
  ...
  function canMoveTopic(topicId: string, parentId: string | null) {
    return canStageTopicMove(items, stagedMoves, topicId, parentId);
  }
  ...
  function stageSelectedMove() {
    ...
    try {
      ...
      workspaceError = error instanceof Error ? error.message : 'Unable to stage the Topic move.';
    }
  }

  function beginTopicDrag(event: DragEvent, topicId: string) {
    if (!organizeMode) return;
    draggedTopicId = topicId;
    draggedCaseIds = [];
    if (event.dataTransfer) {
      event.dataTransfer.effectAllowed = 'move';
      event.dataTransfer.setData('application/x-flashcards-topic', topicId);
      event.dataTransfer.setData('text/plain', topicId);
    }
  }

  function beginCaseDrag(event: DragEvent, caseId: string) {
    if (!organizeMode) return;
    const ids = selectedCaseIds.includes(caseId) && selectedCaseIds.length
      ? selectedCaseIds.slice(0, 60)
      : [caseId];
    selectedCaseIds = ids;
    selectedId = '';
    draggedCaseIds = ids;
    ...
    if (event.dataTransfer) {
      event.dataTransfer.effectAllowed = 'move';
      event.dataTransfer.setData('application/x-flashcards-cases', JSON.stringify(ids));
  ...
  function nodeDropAllowed(row: TaxonomyWorkspaceRow) {
    if (!organizeMode || !row.isActive) return false;
    if (draggedCaseIds.length) return row.kind === 'topic';
    if (draggedTopicId) return canMoveTopic(draggedTopicId, row.id);
    return false;
  }

  function allowNodeDrop(event: DragEvent, row: TaxonomyWorkspaceRow) {
    if (!nodeDropAllowed(row)) return;
    event.preventDefault();
    if (event.dataTransfer) event.dataTransfer.dropEffect = 'move';
  }

  function dropOnNode(event: DragEvent, row: TaxonomyWorkspaceRow) {
    if (!nodeDropAllowed(row)) return;
    event.preventDefault();
    if (draggedCaseIds.length && row.kind === 'topic') {
      stageCasePrimaryTopic(draggedCaseIds, row.id);
      clearDrag();
      return;
    }
    if (draggedTopicId) {
      try {
        stagedMoves = stageTopicMove(items, stagedMoves, draggedTopicId, row.id);
        selectedId = draggedTopicId;
        selectedCaseIds = [];
        workspaceError = '';
      } catch (error) {
        workspaceError = error instanceof Error ? error.message : 'Unable to stage the Topic move.';
  ...
  function allowUnassignedDrop(event: DragEvent) {
    if (!draggedTopicId || !canMoveTopic(draggedTopicId, null)) return;
    event.preventDefault();
    if (event.dataTransfer) event.dataTransfer.dropEffect = 'move';
  }

  function dropUnassigned(event: DragEvent) {
    if (!draggedTopicId || !canMoveTopic(draggedTopicId, null)) return;
    event.preventDefault();
    try {
      stagedMoves = stageTopicMove(items, stagedMoves, draggedTopicId, null);
      selectedId = draggedTopicId;
      selectedCaseIds = [];
      workspaceError = '';
    } catch (error) {
  ...
  function stageCasePrimaryTopic(caseIds: string[], topicId: string) {
    try {
      ...
      workspaceError = error instanceof Error ? error.message : 'Unable to stage the Primary Topic change.';
    }
  }
  ...
  function stageCaseTags(caseIds: string[], tagId: string, operation: 'add' | 'remove') {
    ...
    if (!tag) {
      workspaceError = 'Choose an active Tag.';
      return;
    }
    const selectedCaseRows = workspaceCases.filter((caseItem) => caseIds.includes(caseItem.id));
    try {
      stagedCaseTagChanges = stageCaseTagChanges(caseTagAssignments, stagedCaseTagChanges, selectedCaseRows, tag, operation);
      workspaceError = '';
    } catch (error) {
      workspaceError = error instanceof Error ? error.message : 'Unable to stage the Case Tag change.';
    }
  }
</script>
...
<section class="workspace-shell">
  <div class="toolbar" aria-label="Taxonomy workspace controls">
    ...
    <div class="toolbar-actions">
      ...
      {#if organizeMode}
        <button class="button organize-active" type="button" disabled={stagedChangeCount > 0} onclick={exitOrganize}>Done organizing</button>
      {:else}
  ...
  {#if focusedSystem}
    <div class="focus-banner"><div><span class="muted">Systems &amp; Topics /</span> <strong>{focusedSystem.name}</strong></div><button class="button compact" type="button" onclick={clearFocus}>← All Systems</button></div>
  {/if}

  {#if organizeMode}
    <section class="organize-banner" aria-label="Organize mode">
      <div><p class="eyebrow">Organize mode</p><strong>Hierarchy and Case classification changes are staged together.</strong><p>Move Topics, drag Cases onto Topics, or use the inspectors. Review all pending domains together before applying.</p></div>
      <div class:drop-allowed={Boolean(draggedTopicId && canMoveTopic(draggedTopicId, null))} class="unassigned-drop" role="group" aria-label="Move Topic to Unassigned" ondragover={allowUnassignedDrop} ondrop={dropUnassigned}>Drop Topic here → Unassigned</div>
    </section>
  {/if}
  ...
  {#if createOpen}
    <section class="create-panel" aria-labelledby="create-taxonomy-heading">
      ...
      <form method="POST" action="?/createConcept" class="create-form">
        ...
        {#if createKind === 'topic'}
          <div><SearchableTaxonomyPicker bind:value={createParentId} options={parentSearchOptions} label="Parent" searchPlaceholder="Search System or Topic…" emptyLabel="Unassigned" /><input type="hidden" name="parent_id" value={createParentId} /></div>
        {:else}<input type="hidden" name="parent_id" value="" />{/if}
        ...
        <div class="create-actions"><button class="button" type="button" onclick={() => { createOpen = false; }}>Cancel</button><button class="button primary" type="submit">Create {createKind === 'system' ? 'System' : 'Topic'}</button></div>
      </form>
    </section>
  {/if}
  ...
  <div class="workspace-grid">
    <section class="tree-panel" aria-labelledby="taxonomy-tree-heading">
      <div class="panel-heading"><div><p class="eyebrow">Taxonomy workspace</p><h2 id="taxonomy-tree-heading">Systems &amp; nested Topics</h2><p class="muted">{organizeMode ? 'Cases can be selected or dragged to an active Topic; Tags remain separate from the tree.' : "Browse safely here. Cases stay hidden until you reveal a Topic's direct Cases."}</p></div><span class="result-count">{rows.length} visible{#if selectedCaseIds.length} · {selectedCaseIds.length} selected Cases{/if}{#if stagedChangeCount} · {stagedChangeCount} staged{/if}</span></div>

      {#if rows.length === 0}
        <div class="empty-state">No Systems or Topics match the current search and filter.</div>
      {:else}
        <div class="tree" role="list" aria-label="System and Topic hierarchy">
          {#each rows as row (row.id)}
            <div class="tree-block">
              <div class:selected={selectedId === row.id} class:context-only={row.contextOnly} class:inactive={!row.isActive} class:staged={row.kind === 'topic' && isMoveStaged(row.id)} class:drop-allowed={nodeDropAllowed(row)} class="tree-row" role="listitem" ondragover={(event) => allowNodeDrop(event, row)} ondrop={(event) => dropOnNode(event, row)}>
                <div class="tree-main" style={`padding-left: ${row.depth * 1.05}rem`}>
                  {#if organizeMode && row.kind === 'topic'}<button class="drag-handle" type="button" draggable={true} aria-label={`Drag ${row.name} to move Topic`} title="Drag to stage a Topic move" ondragstart={(event) => beginTopicDrag(event, row.id)} ondragend={clearDrag}>⋮⋮</button>{/if}
                  {#if row.hasChildren}<button class="collapse-button" type="button" aria-label={(collapsedIds.includes(row.id) ? 'Expand ' : 'Collapse ') + row.name} aria-expanded={!collapsedIds.includes(row.id)} onclick={() => toggleCollapsed(row.id)}>{collapsedIds.includes(row.id) ? '▸' : '▾'}</button>{:else}<span class="tree-spacer" aria-hidden="true"></span>{/if}
                  <button class="node-button" type="button" onclick={() => selectNode(row.id)}><span class="node-name">{row.name}</span><span class="node-meta">{row.kind === 'system' ? 'System' : 'Topic'}{#if row.unassigned} · Unassigned{/if}{#if !row.isActive} · Inactive{/if}{#if row.kind === 'topic' && isMoveStaged(row.id)} · Move staged{/if}</span></button>
                </div>
                <div class="node-counts">{#if row.kind === 'topic'}<span><strong>{row.directCaseCount}</strong> direct</span>{#if row.descendantStudyCaseCount !== row.directCaseCount}<span><strong>{row.descendantStudyCaseCount}</strong> subtree</span>{/if}{#if row.directSubtopicCount}<span><strong>{row.directSubtopicCount}</strong> subtopics</span>{/if}{:else}<span><strong>{row.descendantStudyCaseCount}</strong> study Cases</span>{#if row.directSubtopicCount}<span><strong>{row.directSubtopicCount}</strong> Topics</span>{/if}{/if}</div>
                <div class="row-actions">
                  {#if organizeMode && row.kind === 'topic'}<button class="text-action" type="button" onclick={() => openMove(row)}>Move to…</button>{:else if row.isActive}<button class="text-action" type="button" onclick={() => createChild(row)}>{row.kind === 'system' ? '+ Add Topic' : '+ Add subtopic'}</button>{/if}
                  {#if row.kind === 'topic' && row.directCaseCount > 0}<button class="text-action" type="button" aria-pressed={casesVisible(row)} onclick={() => toggleCases(row.id)}>{casesVisible(row) ? 'Hide Cases' : 'Show Cases'}</button>{/if}
              ...
              {#if row.kind === 'topic' && casesVisible(row)}
                <div class="case-list" style={`margin-left: ${Math.max(0, row.depth + 1) * 1.05 + 2.15}rem`}>
                  {#if row.directCases?.length}
                    {#if organizeMode && row.directCases.length > 1}<div class="case-list-tools"><span>{row.directCases.length} direct Cases</span><button class="text-action" type="button" onclick={() => selectDirectCases(row)}>Select direct Cases</button></div>{/if}
                    {#each row.directCases as caseItem (caseItem.id)}
                      {#if organizeMode}
                        <div class:selected-case={selectedCaseIds.includes(caseItem.id)} class:staged-case={isCasePrimaryChangeStaged(caseItem.id) || isCaseTagChangeStaged(caseItem.id)} class="case-row organize-case-row">
                          <button class="case-drag" type="button" draggable={true} aria-label={`Drag ${caseItem.title} to change Primary Topic`} title="Drag Case onto a Topic to stage its Primary Topic change" ondragstart={(event) => beginCaseDrag(event, caseItem.id)} ondragend={clearDrag}>⋮⋮</button>
                          <input class="case-checkbox" type="checkbox" checked={selectedCaseIds.includes(caseItem.id)} aria-label={`Select ${caseItem.title}`} onchange={() => toggleCaseSelection(caseItem.id)} />
                          <button class="case-select" type="button" onclick={() => selectOnlyCase(caseItem.id)}><span class="case-title">{caseItem.title}</span><span class="case-meta">{isCasePrimaryChangeStaged(caseItem.id) ? 'Primary Topic change staged' : isCaseTagChangeStaged(caseItem.id) ? 'Case Tag change staged' : 'Primary Topic: ' + row.breadcrumbLabel}</span></button>
  ...
  {#if stagedChangeCount}
    <StagedTaxonomyReview moves={stagedMoves} caseChanges={stagedCaseChanges} tagChanges={stagedCaseTagChanges} items={projectedItems} onDiscardAll={discardAllChanges} onUndoMove={undoMove} onUndoCaseChange={undoCaseChange} onUndoTagChange={undoTagChange} />
  {/if}
</section>
...
<style>
  ...
  .workspace-grid { display: grid; grid-template-columns: minmax(0, 1.85fr) minmax(300px, .8fr); gap: 1rem; align-items: start; }
  ...
  .drag-handle,.case-drag { flex: 0 0 1.75rem; width: 1.75rem; height: 1.75rem; padding: 0; border: 1px solid #d0d5dd; border-radius: 5px; background: #fff; color: #667085; cursor: grab; font: inherit; font-weight: 800; line-height: 1; }
  .drag-handle:active,.case-drag:active { cursor: grabbing; }
  ...
  @media (max-width: 1120px) { .tree-row { grid-template-columns: minmax(0, 1fr) auto; } .row-actions { grid-column: 1 / -1; padding-left: 1.9rem; justify-content: flex-start; } .move-panel { grid-template-columns: 1fr; } .move-actions { justify-content: flex-start; } }
  @media (max-width: 920px) { .toolbar,.workspace-grid,.organize-banner { grid-template-columns: 1fr; } .toolbar-actions { justify-content: flex-start; } .create-form { grid-template-columns: 1fr; } .create-form .wide,.create-actions { grid-column: auto; } }
  @media (max-width: 620px) { .tree-row { grid-template-columns: 1fr; gap: .35rem; } .node-counts,.row-actions { justify-content: flex-start; padding-left: 1.9rem; } .case-list { margin-left: 2rem !important; } .case-list-tools,.focus-banner,.panel-heading { align-items: flex-start; flex-direction: column; } .organize-case-row { grid-template-columns: auto minmax(0, 1fr); } .organize-case-row .case-drag { display: none; } .organize-case-row .case-select { grid-column: 2; } .organize-case-row .case-open { grid-column: 2; } .drag-handle { display: none; } }
</style>
