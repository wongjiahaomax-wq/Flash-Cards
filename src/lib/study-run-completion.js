...
/**
 * Active-Review completion orchestration shared by learner Study and the
 * local-only FSRS regression preview. A matching learner-owned browser
 * descriptor is intentionally routed to the receipt-owning completion service
 * before reading active_reviews, so an identical retry remains safe after the
 * first transaction consumed the active Review and its HTTP response was lost.
 *
 * Browser run state is disposable: if it is missing or cannot be advanced, the
 * persisted completion still wins and the caller receives runLost=true.
 *
 ...
 * @returns {Promise<any>}
 */
export async function completeStudyRunRequest(input, services) {
