...
- During active implementation, use the cheapest feedback that meaningfully tests the current risk: Vite/HMR for presentation-only iteration and the nearest directly related test(s) for logic changes. For maintained Node tests, use the repository-owned compact path `npm test -- <test-file>` rather than raw `node --test` unless explicitly diagnosing the runner itself.
- After a coherent change, run `npm run agent:checks` (compact by default; use `npm run agent:checks -- --verbose` only when additional detail is needed) to obtain current iteration/checkpoint guidance, final required checks, and specialized requirements.
