...
- Preserve the CI Node-test reporting contract. `npm test` remains the canonical complete `node --test` command; ordinary CI may add `--test-reporter=./scripts/ci-test-reporter.mjs` only through the CI wrapper. The reporter must consume structured `node:test` events, keep successful-test output to compact progress plus one final summary, collect real failures prominently at the end, retain test name, file/line, failure type, error/code, operator, expected/actual values and useful stack frames, and emit GitHub file/line error annotations. Do not regress to buffering/parsing TAP, dot, or spec text, and do not restore hundreds of successful per-test records to ordinary CI logs.
