...
- Workflow YAML orchestrates PR state/check execution; it does not own changed-path classification or validation satisfaction rules. `scripts/agent-checks-lib.mjs` is the one changed-path classifier, and `scripts/validation-contract.mjs` owns named checks plus explicit satisfaction/deduplication. Do not reimplement either contract with workflow path conditions or shell conditionals. See `docs/TESTING_AND_VALIDATION_GUIDANCE.md`.
