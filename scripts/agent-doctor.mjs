...
import { branchStatus, findRepositoryRoot, nodeMajorStatus, overallDoctorStatus, wranglerVersionStatus } from './agent-doctor-lib.mjs';
...
const nodeCheck = nodeMajorStatus(process.version, EXPECTED_NODE_MAJOR);
...
console.log('');
console.log(`Node               ${process.version}  ${nodeCheck.ok ? 'OK' : `ERROR (expected major ${EXPECTED_NODE_MAJOR})`}`);
...
if (!nodeCheck.ok) console.error(`\nERROR: Node ${EXPECTED_NODE_MAJOR}.x is required. Switch Node versions, then rerun npm ci.`);
