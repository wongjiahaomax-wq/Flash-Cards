...
// Bounded bootstrap exception for the independently identified run-20 application
// restored during the 2026-09-05 incident. Any other Worker must identify itself
// from the public edge through Cloudflare Version Metadata.
...
function assertStoredIdentity(identity, expectedVersion) {
  const expected = requireWorkerVersion(expectedVersion, 'expected Worker version');
  if (identity == null && expected === LEGACY_BOOTSTRAP_LKG_VERSION) {
    return evaluateWorkerIdentity({ status: null, body: null }, expected);
  }
...
export function buildRecoveryRecord({
  ...
  return {
    ...
    source: { run_id: sourceRunId, run_attempt: sourceRunAttempt, sha: sourceSha },
...
async function main() {
  ...
  if (command === 'write-record') {
    ...
    const versionBefore = requiredOption(options, 'version-before');
    const probe = JSON.parse(fs.readFileSync(requiredOption(options, 'probe-file'), 'utf8'));
    const identity = options['identity-file']
      ? JSON.parse(fs.readFileSync(options['identity-file'], 'utf8'))
      : await probeWorkerIdentity(baseUrl, versionBefore);
