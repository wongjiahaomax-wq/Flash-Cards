...
`learner_feedback` is Case-level, learner-owned report data. It stores the active Review-derived Case and learner identifiers, reporter and Case-title snapshots, plain-text body, lifecycle status (`open`, `resolved`, or `dismissed`), submission time, and nullable review metadata. Case references are restrictive so deactivation preserves reports. Permanent learner-account deletion removes feedback during the existing bounded terminal profile sweep; self-service study-data deletion does not touch this table.
