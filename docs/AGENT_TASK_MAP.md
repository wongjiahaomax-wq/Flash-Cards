...
For Preview backend work, keep caller imports on `src/lib/server/db/preview-workspace.js`. Preview Session lookup/creation and TTL live in `src/lib/server/db/preview-workspace/session.js`; ownership/security guards live in `src/lib/server/db/preview-workspace/ownership.js`; shared Preview error/input primitives live beside them. `ensurePreviewWorkspace()` and full cleanup coordination remain in the public façade until the staged cleanup refactor, so future ownership or Session work should start from the focused foundation module plus the façade integration point rather than loading the full Preview backend by default.
