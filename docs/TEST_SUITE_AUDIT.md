...
- **Rewritten to stronger executable owner:** layout preference normalization, read/write semantics, persistence and storage-failure fallback remain directly executed by `test/admin-case-editor-layout.test.js`. The responsive contract no longer freezes the exact `$state('compact')` default literal, exact storage-helper invocation spelling, direct `window.localStorage` negative checks, or exact `setEditorLayout(...)` implementation text merely to prove helper behavior. The route retains only thin integration/data-flow assertions proving the tested helpers are actually wired to mount and in-place layout changes.
- **Retained/hardened as composition owner:** the old literal absence check for `ClassicCaseEditor|CompactCaseEditor` is replaced with a structural rule. The test discovers the shared `$lib/components/case-editor/...` component family, requires each imported shared component to render exactly once in the root editor, and rejects any of those shared components appearing inside an `editorLayout` conditional. This still fails if Classic/Compact grows separate shared authoring trees without freezing future component names.
...
- **Rewritten away from incidental form tokens:** exact `rows="3"` is replaced with comparable Prompt/Answer initial row sizing. Exact auto-grow helper names and exact numeric `maxHeight` values are replaced by following the action actually wired to each field, proving finite bounded growth and expand/collapse behavior, and requiring image-question growth limits to remain smaller than the main Answer editor's limit.
- **Rewritten away from incidental responsive tokens:** exact `@media (min-width: 1024px)`, exact `2fr / 3fr`, exact sticky-navigation declaration text and exact `scroll-margin-top: 4.75rem` are retired. The test scopes assertions to actual CSS rule bodies, finds wide/narrow viewport classes structurally, requires multiple Prompt/Answer tracks when wide, one track when narrow, the same wide class for sticky navigation, and positive anchor clearance.
...
Implementation-only head `7a47f6dceb841c2764d632a9f76a307e51756754` passed Draft CI #1343 before the documentation reconciliation: 110 maintained / 104 selected / 6 excluded, 629/629 fast Node tests, 0 Svelte errors / 5 existing warnings, ECG 6/6, taxonomy 3/3, slide-review 23/23 and slide-review build. Wrangler runtime-smoke #174 also passed. Exact-head post-documentation CI remains the final tranche gate.
...
| Case-editor responsive contract | FIRST BOUNDED CHECKPOINT 5 TRANCHE: helper/storage semantics consolidated into direct executable owner; single-tree composition, in-place switch integration, form/scope/reorder reachability, exact scroll restoration and minimum responsive layout remain focused source/data-flow owners without exact breakpoint/helper/CSS locks |
