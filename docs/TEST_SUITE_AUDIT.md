...
The recurring independent-review lesson is that a thinner source contract must still prove the exact boundary it claims to own. Checkpoint 6 adds a second lesson: deleting or specializing a small number of cheap files is not a substitute for profiling expensive ordinary suites and separately benchmarking candidate setup or runner-level mechanisms.
...
1. **DB-heavy suites are measured as expensive; repeated current-schema/SQLite fixture setup is the strongest benchmark candidate.** Several top-ranked ordinary suites create fresh in-memory SQLite databases, run `applyCurrentSchema(...)`, and often seed data repeatedly. The profiler measured whole-file durations rather than separately timing database creation, schema application, seeding, or domain/database operations, so it does not establish which component dominates. Code inspection makes repeated setup the strongest follow-up hypothesis. `test/current-schema.js` deliberately executes the concatenated complete migration authority into each fresh database, and that correctness isolation should remain. A separately reviewed benchmark could test pre-migrated fixture snapshots/cloning or transaction-reset helpers while preserving isolation and current-schema authority. Classification: **measurement-backed suite hotspot with a benchmark hypothesis; not an exclusion candidate**.
