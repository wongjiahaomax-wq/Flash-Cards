...
The recurring independent-review lesson is that a thinner source contract must still prove the exact boundary it claims to own. Checkpoint 6 adds a second lesson: deleting or specializing a small number of cheap files is not a substitute for profiling the dominant ordinary-fixture and runner costs.
...
Median summed excluded-file time was **348.2 ms**, about **1.66%** of the corresponding suite wall time. The median gap between suite duration and the sum of reported per-file durations was about **1.7318 s**, consistent with material runner/file-process/startup/coordination cost outside the reporter's file bodies. This is an attribution clue, not a claim that all of that gap is one specific mechanism.
...
2. **Distributed file/runner startup and fragmentation — secondary candidate.** There are 110 maintained files, median reported file duration is about 70 ms, and about 1.6–2.0 s per profiled run sits outside summed reported file duration. A follow-up may benchmark file aggregation or fixture/helper reuse where ownership remains clear, but must not mechanically merge unrelated contracts or weaken parallel isolation. Classification: **safe-performance-optimization candidate requiring benchmark**.
