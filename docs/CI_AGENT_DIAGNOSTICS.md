...
For Node tests, `scripts/ci-test-reporter.mjs` remains the structured `node:test` presentation layer. It emits one `CI_ERROR` record per real test failure, deduplicated file-level `CI_REPRO` commands, and a final failed-test count after the existing detailed failure section. Todo failures are not promoted to real failures.
