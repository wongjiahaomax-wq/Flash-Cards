...
- **Explicit staging readiness gate (blocker):** The current job is inserted with `status='validating'` before staging, and ordinary `status='failed'` jobs can be resumed. Do not rely on ZIP/plan existence, successful response delivery, or best-effort cleanup as proof of complete staging. Represent pre-completion staging durably using the existing `phase` field (for example, `staging` while starting and `staging_failed` on failure; existing schema permits text phases), and advance to the first validation phase **only after every required R2 PUT succeeds**. A Worker interruption leaves staging unprocessable. The server-side process/claim path must reject incomplete-staging phases even on a direct request, without validation or domain writes. Do not allow ordinary Retry/Resume for staging failures; an administrator may start a new import from the exact reviewed ZIP. Preserve Retry/Resume for jobs that completed staging and later failed during validation/import.
- **Cancellation versus active staging (blocker):** Use the existing D1 lease/token or an equivalently minimal conditional job-state guard to claim the job *before the first staging PUT* and retain exclusive ownership until every started PUT has settled and compensation has finished. A cancel request while staging is active must neither change the job to `cancelled` nor delete any staged object, including when a lease timestamp has expired; return a clear busy/deferred result. On successful staging, conditionally advance to validation and release ownership only if the job is still in the claimed staging state; a cancelled job must never advance. On failure, settle in-flight writes and perform compensation before releasing the staging claim. Once staging is fully settled, allow the existing cancellation/cleanup path. If the Worker is interrupted and ownership remains unresolved, keep the job unprocessable and staging objects private; do **not** assume lease expiry proves that an earlier upload has stopped, and do not delete objects until no writer can still be active. Permit a fresh import under a new job ID while this stranded job is retained for safe later cleanup. Keep this narrowly scoped; no schema migration, background cleanup machinery, or speculative recovery framework.
...
3. **Actual runtime/process-action regression:** Hold a staging PUT pending; prove a direct process request cannot validate/write and a concurrent cancel request cannot mark the job cancelled or delete staging objects. Include a stale/expired staging-lease case: expiry alone must not permit deletion while an earlier PUT remains in flight. Settle the PUTs and prove the job can then finish staging or fail and compensate, with cancellation safe only after settlement. Verify staging success transitions conditionally to validation, a cancelled job cannot transition into validation, incomplete/failed staging cannot be resumed even after partial cleanup failure, interrupted staging stays unprocessable, and a later validation/import failure retains normal resumability. Cover the Admin Retry/Resume affordance for staging failures with the smallest appropriate UI/controller test. Reuse existing runtime-safety tests for unrelated lease/checkpoint behavior rather than introducing new architecture.
